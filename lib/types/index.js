...
function sessionContext(exec) {
    if (exec.agent === undefined)
        throw new Error('file-memory tools require an Agent-backed session');
...
export function apply(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        name: 'memorize',
        description: 'Append key facts to this session\'s notes file (workspace `.dsh-notes/<session>.md`), stored verbatim and '
        ...
        async execute(args, exec) {
            ...
            const fs = fsOf();
            if (fs === undefined)
                return { ok: false, reason: 'fs service unavailable in this deployment' };
    ...
    ctx.tools.register(defineTool({
        name: 'recall',
        description: 'Read this session\'s notes file back, optionally filtered to lines containing `query`. Call AFTER compaction '
        ...
        async execute(args, exec) {
            ...
            const fs = fsOf();
            if (fs === undefined)
                return { ok: false, found: false, reason: 'fs service unavailable in this deployment' };
            const { id, cwd } = sessionContext(exec);
            const target = await resolveNotes(fs, notesDir, id, cwd);
            const lines = await existingLines(fs, target);
            if (lines.length === 0)
                return { ok: true, found: false, note: 'no notes yet for this session' };
